window.Pawear = (() => {
  ...
  document.addEventListener('click',event=>{
    const add=event.target.closest('[data-add-cart]'); if(add){run(async()=>{if(add.dataset.needsSize==='true'){location.href=add.dataset.productUrl;return;}const size=add.dataset.sizeSelector?document.querySelector(add.dataset.sizeSelector).value:undefined;if(add.dataset.sizeSelector&&!size)throw new Error('ابتدا سایز جوراب را انتخاب کن.');const qty=add.dataset.quantity?Number(document.querySelector(add.dataset.quantity).value):1;if(!Number.isInteger(qty)||qty<1||qty>100)throw new Error('تعداد باید بین ۱ تا ۱۰۰ باشد.');const result=await request('/api/cart','POST',{itemType:add.dataset.type,item:add.dataset.addCart,quantity:qty,...(size?{size}:{})});document.querySelectorAll('[data-cart-count]').forEach(el=>el.textContent=result.data.cart.items.reduce((n,i)=>n+i.quantity,0));notice('به سبد خرید اضافه شد.');})(event);return;}
