...
const fields = {
    sizes:Joi.array().max(30).items(Joi.object({label:Joi.string().trim().min(1).max(30).required(),price:Joi.number().integer().min(1).max(1000000000).required(),stock:Joi.number().integer().min(0).max(1000000).required(),isActive:Joi.boolean().default(true)})),
