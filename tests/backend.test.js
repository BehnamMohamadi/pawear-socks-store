...
test('PAWEAR backend integration with isolated replica-set database', async (t) => {
    ...
    await t.test('V2 shipping settings enforce admin access, mixed maximum and frozen paid totals',async()=>{
        ...
        await Settings.init();
        assert.equal((await request('/api/admin/settings/shipping','PUT',{sockAmount:45000,boxAmount:70000},userCookie)).status,403);
        assert.equal((await request('/api/admin/settings/shipping','PUT',{sockAmount:-1,boxAmount:70000},adminCookie)).status,400);
        assert.equal((await request('/api/admin/settings/shipping','PUT',{sockAmount:45000,boxAmount:70000},adminCookie)).status,200);
        ...
        await request('/api/cart','DELETE',null,userCookie);
        const p=await Product.create({name:'ارسال نسخه دو',category:category._id,subCategory:sub._id,price:90000,stock:10});
        ...
        await request('/api/admin/settings/shipping','PUT',{sockAmount:50000,boxAmount:70000},adminCookie);
    ...
    await t.test('V2 independent sizes, box components, reservation release and dispatch are synchronized',async()=>{
        await request('/api/cart','DELETE',null,userCookie);
        const body={name:'جوراب سایزبندی نسخه دو',category:String(category._id),subCategory:String(sub._id),price:1,sizes:[{label:'۴۰ - ۴۳',price:100000,stock:5},{label:'43-46',price:150000,stock:3}]};
        ...
        const p=created.data.data.product;assert.equal(p.price,100000);assert.equal(p.stock,8);assert.equal(p.sizes[0].label,'40-43');
        const dup=await request('/api/products','POST',{...body,name:'سایز تکراری',sizes:[body.sizes[0],{...body.sizes[1],label:'40-43'}]},adminCookie);assert.equal(dup.status,400);
        assert.equal((await request('/api/cart','POST',{itemType:'Product',item:p._id,quantity:1},userCookie)).status,400);
        ...
        let row=await Product.findById(p._id);assert.deepEqual(row.sizes.map(v=>v.stock),[3,1]);assert.equal(row.stock,4);
        assert.equal((await request('/api/products/'+p._id,'PATCH',{sizes:p.sizes.map(({label,price,stock,isActive})=>({label,price,stock,isActive}))},adminCookie)).status,409);
        await Payment.updateOne({_id:paymentId},{$set:{expiresAt:new Date(Date.now()-1000)}});await expireStalePayments();
        row=await Product.findById(p._id);assert.deepEqual(row.sizes.map(v=>v.stock),[5,3]);assert.equal(row.stock,8);
        ...
        const productPage=await fetch(base+'/product/'+encodeURIComponent(p.slug));assert.match(await productPage.text(),/id="productSize"/);
        const editPage=await fetch(base+'/admin/products/'+p._id+'/edit',{headers:{Cookie:adminCookie}});assert.match(await editPage.text(),/data-size-row/);
        const boxForm=await fetch(base+'/admin/boxes/'+b.data.data.box._id+'/edit',{headers:{Cookie:adminCookie}});assert.match(await boxForm.text(),/data-size="43-46"/);
    });
    await t.test('V2 concurrent purchases reserve only one last size without touching another size',async()=>{
        const p=await Product.create({name:'آخرین سایز نسخه دو',category:category._id,subCategory:sub._id,price:1,sizes:[{label:'S',price:80000,stock:1},{label:'L',price:95000,stock:7}]});
